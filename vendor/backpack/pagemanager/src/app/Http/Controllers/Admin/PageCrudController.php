...
class PageCrudController extends CrudController
{
    ...
    protected function setupListOperation()
    {
        ...
        if(!request('templates'))
            $this->crud->query->where('template', 'common');

        $this->crud->addFilter([
            'name' => 'templates',
            'type' => 'simple',
            'label' => 'Показать шаблоны',
          ],
          false,
          function() {
            $this->crud->query->where('template', '!=', 'common');
            $this->crud->denyAccess('delete');
            $this->crud->denyAccess('translate');
          });
    ...
    public function addDefaultPageFields($template = false)
    {
        ...
        if($this->crud->getCurrentEntry() && $this->crud->getCurrentEntry()->template == 'common') {
            $this->crud->addField([
                'name' => 'translation_id',
