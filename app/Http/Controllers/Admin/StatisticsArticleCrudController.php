...
use App\Http\Requests\StatisticsArticleRequest;
...
/**
 * Class StatisticsArticleCrudController
 * @package App\Http\Controllers\Admin
 * @property-read \Backpack\CRUD\app\Library\CrudPanel\CrudPanel $crud
 */
class StatisticsArticleCrudController extends CrudController
{
    ...
    public function setup()
    {
        ...
        $this->crud->setRoute(config('backpack.base.route_prefix') . '/statisticsarticle');
        ...
        if (!Cache::has('statistics_article_years')) {
          $expiresAt = Carbon::now()->addWeek();

            foreach(Article::whereHas('category', function($q) {
                $q->whereIn('parent_id', $this->ids);
            })->pluck('id', 'date') as $date => $id) {

                $year = explode('-', $date)[0];
                $years[$year] = $year;
            }

          Cache::put('statistics_article_years', $years, $expiresAt);
        } else {
            $years = Cache::get('statistics_article_years');
        }
    ...
    {
        $this->crud->setValidation(StatisticsArticleRequest::class);
