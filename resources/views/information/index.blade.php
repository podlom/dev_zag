    ...
    <section class="breadcrumbs">
        <div class="breadcrumbs__wrapper">
            <div class="breadcrumbs__list">
                ...
                <a href="#" class="breadcrumbs__link" @click.prevent="query.category = null" v-cloak>{{ __('main.Полезная информация') }}</a>
        ...
        <div class="news__wrapper container">
            <div class="general-heading">
                <h1 class="main-caption-l" v-cloak>
                    ...
                    @{{ categories[query.category] }} <span v-if="query.page > 1"> ➨ {{ __('main.страница') }} @{{ query.page }}</span>
                    </template>
                    <template v-else>
                    {{ __('main.Полезная информация') }} <span v-if="query.page > 1"> ➨ {{ __('main.страница') }} @{{ query.page }}</span>
                    </template>
                </h1>
...
<script>
  ...
  var parentCategorySlug = @json($parent_category_slug);
  var seo_text = @json($seo_text);
  var content = @json($content);
  var page = @json(request('page')? request('page') : 1);
</script>
</script>
<script src="{{ url('js/information/index.js?v=' . $version) }}"></script>
@endpush