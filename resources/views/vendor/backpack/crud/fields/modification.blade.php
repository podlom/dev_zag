...
@endphp
<div id="mod_field" style="width: 100%">
...
<div class="@if(config('aimix.shop.enable_modifications')) default-modifications @endif" style="display:none">
  ...
  <div class="form-group col-sm-12">
    <label for="old_price">Макс. цена</label>
...
<div class="col-sm-12">
  ...
  <div class="card" v-for="(item, index) in items" >
    ...
    @if(!$base || !$base->original)
            ...
            <div class="input-group">
              ...
              <div class="row">
                  <div class="col-sm-6" style="margin-bottom: 20px;">
                      <img :src="item.previews && item.previews[key]? item.previews[key] : (value.indexOf('http') === -1 && value.indexOf('/') != 0? '/' + value : value)" style="height: 200px;max-width:none">
                  </div>
              </div>
              <div class="btn-group">
      ...
      <div class="form-group col-sm-12" >
        <label for="uniqId">Планировки</label>
        <!-- <draggable v-model="item.images" group="items" @start="drag=true" @end="drag=false"> -->
        <div class="form-group d-flex flex-wrap justify-content-between" v-for="(value, key) in item.layouts">
          <div class="form-group" style="width: 100%">
            <label class="input-group">
              <input type="text" v-model="value.name" class="form-control" :name="'{{ $field['name'] }}[' + (index + 1) + '][layouts][' + key + '][name]'" placeholder="Название планировки">
            </label>
            <div class="input-group">
              <div>
              <!-- Wrap the image or canvas element with a block element (container) -->
              <div class="row">
                  <div class="col-sm-6" style="margin-bottom: 20px;">
                      <img :src="item.layout_previews && item.layout_previews[key]? item.layout_previews[key] : (value && value.image? '/' + value.image : '')" style="height: 200px;max-width:none">
                  </div>
              </div>
              <div class="btn-group">
                  <div class="btn btn-light btn-sm btn-file">
                      Выберите файл <input type="file" class="hide" :name="'{{ $field['name'] }}[' + (index + 1) + '][layouts][' + key + '][image]'" @change="fileChangeLayouts($event, index, key)">
                      <input type="hidden" :name="'{{ $field['name'] }}[' + (index + 1) + '][layouts][' + key + '][image]'" :value="value.image" v-if="!item.layout_previews || (item.layout_previews && !item.layout_previews[key])">
                  </div>
                  <div class="btn btn-danger btn-sm option-delete" @click="removeLayout(index, key)">Удалить</div>
                </div>
              </div>
            </div>
        ...
        <!-- </draggable> -->
        <div class="input-group" style="margin-top: 15px">
          <div class="btn btn-primary option-add" @click="addLayout(index)"><i class="fa fa-plus"></i> Добавить планировку</div>
        </div>
      </div>
...
<script src="{{ url('/packages/aimix/shop/js/fields/modification.js?v=1.2') }}"></script>
