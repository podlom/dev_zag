        ...
        <td>
          ...
          <td>
            @{{ region.status_done }}
					...
					<td>
            @{{ region.status_done_flats }}
					</td>
          <td>
            @{{ region.status_project }}
          </td>
					<td>
            @{{ region.status_project_flats }}
					</td>
          <td>
            @{{ region.status_building }}
          </td>
					<td>
            @{{ region.status_building_flats }}
					</td>
					<td>
            @{{ region.total_flats }}
					</td>
				...
				<template v-for="(area, areaKey) in region.areas">
					<tr style="background-color: #D3D3D3" v-show="opened.regions.includes(regionKey)" @click="opened.areas.includes(areaKey)? opened.areas.splice(opened.areas.indexOf(areaKey), 1) : opened.areas.push(areaKey)">
						...
						<td>
							@{{ area.status_done_flats }}
						</td>
						<td>
							@{{ area.status_project }}
						</td>
						<td>
							@{{ area.status_project_flats }}
						</td>
						<td>
							@{{ area.status_building }}
						</td>
						<td>
							@{{ area.status_building_flats }}
						</td>
						<td>
							@{{ area.total_flats }}
						</td>
					...
					<template v-for="(city, cityKey) in area.cities">
						<tr v-show="opened.areas.includes(areaKey) && opened.regions.includes(regionKey)" class="cities">
							...
							<td>
								@{{ city.status_done_flats }}
							</td>
							<td>
								@{{ city.status_project }}
							</td>
							<td>
								@{{ city.status_project_flats }}
							</td>
							<td>
								@{{ city.status_building }}
							</td>
							<td>
								@{{ city.status_building_flats }}
							</td>
							<td>
								@{{ city.total_flats }}
							</td>
			...
			<tr class="title">
				...
				<td>
						@{{ table.total.status_done_flats }}
				</td>
				<td>
					@{{ table.total.status_project }}
				</td>
				<td>
						@{{ table.total.status_project_flats }}
				</td>
				<td>
					@{{ table.total.status_building }}
				</td>
				<td>
						@{{ table.total.status_building_flats }}
				</td>
				<td>
						@{{ table.total.total_flats }}
				</td>
				<td>
					@{{ table.total.frozen }}
				</td>
