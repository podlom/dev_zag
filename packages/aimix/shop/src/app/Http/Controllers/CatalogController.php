...
class CatalogController extends \App\Http\Controllers\Controller
{
  ...
  private $is_sale = false;
    ...
    $this->products = Product::active();
		...
		if($this->search_value)
		{
			if($this->search_type == 'name')
				$this->products = $this->products->where('name', 'like', '%'.$this->search_value.'%');
			elseif($this->search_type == 'brand')
				$this->products = $this->products->whereHas('brand', function(Builder $query){
					$query->where('name', 'like', '%'.$this->search_value.'%');
				});
		}else
    $this->products = $this->products->where('products.id', '!=', null);
    ...
    if($this->is_new)
      $this->products = $this->products->where('is_new', 1);

    if($this->is_sale) {
      $this->products = $this->products->whereHas('modifications', function(Builder $query) {
        $query->where('is_default', 1)->where('old_price', '!=', null);
      });
    }
    ...
    if($category_slug && Category::where('slug', $category_slug)->first()->parent_id) {
      $this->products = $this->products->whereHas('category', function(Builder $query) use ($category_slug) {
        $query->where('slug', $category_slug);
      });
    } elseif($category_slug && !Category::where('slug', $category_slug)->first()->parent_id) {
      $this->products = $this->products->whereHas('category', function(Builder $query) use ($category_slug) {
        $query->whereHas('parent', function(Builder $query2) use ($category_slug) {
          $query2->where('slug', $category_slug);
        });
      });
    }
    ...
    $categories = Category::noEmpty()->get();

    $currentCategoryName = $category_slug? Category::where('slug', $category_slug)->first()->name : trans('main.all_products');
    $currentCategorySlug = $category_slug;
		...
		if($this->isJson)
			...
			return view('catalog.index')
               ...
               ->with('currentCategoryName', $currentCategoryName)
               ->with('currentCategorySlug', $currentCategorySlug);
    }
    ...
    private function setPropertiesFromRequest($request){
      ...
      if($this->category) {
        ...
        $this->all_filters = [
          'attributes' => Category::first()->attributes()->where('in_filters', 1)->get(),
          // 'countries' => Country::noEmpty()->get()
      ];
      }
    ...
    public function show(Request $request, $category_slug, $slug) {
      ...
      $same_category_products = Product::where('category_id', $category_id)->where('slug', '!=', $slug)->get();
      
      $reviews = $product->reviews()->where('is_moderated', 1)->orderBy('created_at', 'desc')->get();

      return view('catalog.show')->with('product', $product)->with('attribute_groups', $attribute_groups)->with('same_category_products', $same_category_products)->with('in_stock', $in_stock)->with('reviews', $reviews);
    }
}
